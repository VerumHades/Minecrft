...
template <typename T> static inline void swapToOrder(T& a, T& b) {
    if (a > b) {
        T temp = a;
        a      = b;
        b      = temp;
...
void GameModeStructureCapture::Initialize() {
    ...
    GetBaseLayer().onEntered = [this]() { UICore::get().setFocus(structure_selection); };
}
...
template <typename T> static inline std::string vecToString(T vec, const std::string& separator = " ") {
...
void GameModeStructureCapture::UpdateStructureDisplay() {
    ...
    mode_label->setText("Current mode: " + std::string(structure_menu_mode == CAPTURE ? "Capture" : "Place"));
    structure_capture_start_label->setText("Capture start: " + vecToString(structureCaptureStart));
    structure_capture_end_label->setText("Capture end: " + vecToString(structureCaptureEnd));
    ...
    if (structure_menu_mode == CAPTURE) {
        ...
        if (!selected_structure)
            return;
        state.wireframe_renderer.setCube(1,
...
void GameModeStructureCapture::UpdateStructureSavingDisplay() {
    ...
    save_size_label->setText("Size: " + vecToString(size, "x"));

    blocks_total_label->setText("Blocks total: " + std::to_string(size.x * size.y * size.z));
