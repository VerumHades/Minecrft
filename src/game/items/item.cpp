...
ItemPrototype::ItemPrototype(std::string name, const BlockRegistry::BlockPrototype* prototype) : name(name) {
    if (prototype->render_type != BlockRegistry::BILLBOARD)
        display_type = BLOCK;
    is_block = true;
    ...
    if (prototype->render_type == BlockRegistry::BILLBOARD)
        model = std::make_shared<SpriteModel>(prototype->texture_paths[0]);
    else
        ...
        xml_for_each_child_as(item_definition, item_attribute) {
            ...
            if (attr_name == "name") {
                ...
                xml_for_each_child_as(item_attribute, material_effectiveness) {
                    effectiveness.push_back(XMLExtras::Load<ItemPrototype::ToolEffectiveness>(
...
LogicalItemInventory::LogicalItemInventory(int slots_horizontaly, int slots_verticaly)
    : slots(slots_horizontaly * slots_verticaly), slots_horizontaly(slots_horizontaly),
      slots_verticaly(slots_verticaly) {}
...
void DroppedItem::setup(Entity* entity) {
    ...
    if (entity->getModelInstance())
        entity->getModelInstance()->Scale(glm::vec3(0.3, 0.3, 0.3));
